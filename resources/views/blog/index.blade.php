    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            ...
            <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                <div>
                    <p class="fw-semibold fs-18 mb-0">{{$page_title}}</p>
                    ...
                    <div class="col-md-6 blog-post">
                        <a href="{{ route('post.show', $post->id) }}" class="text-decoration-none text-dark">
                            <div class="card h-100">
                                @if($post->image)
                                <img src="{{ asset($post->image) }}" class="card-img-top blog-image" alt="{{ $post->title }}">
                                @endif
                                <div class="card-body">
                                    <h5 class="card-title post-title">{{ $post->title }}</h5>
                                    <p class="card-text post-body">{!! \Illuminate\Support\Str::limit(base64_decode($post->body), 100, '...') !!}</p>
                                    <div class="mt-0 mb-3 text-dark">
                                        <small><i>By {{ $post->author }}</i></small><br/>
                                        <small><i>Created on {{ $post->created_at->format('F j, Y') }}</i></small><br/>
                                        <small><i>Last updated on {{ $post->updated_at->format('F j, Y') }}</i></small>
                                    </div>
                                
                                    <div class="d-flex">
                                        <a href="{{ route('post.show', $post->id) }}" class="btn btn-info me-2"><i class="ri-edit-2-line"></i></a>
                                        <form action="{{ route('post.destroy', $post->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger show-alert-delete-box"><i class="ri-delete-bin-2-line"></i></button>
                                        </form>
                                    </div>
...
<script type="text/javascript">
    $('.show-alert-delete-box').click(function(event){
        ...
        swal({
            title: "Are you sure?",
            text: "Blog Post Will be Deleted.",
            icon: "warning",
            type: "warning",
            buttons: ["Cancel","Yes!"],
